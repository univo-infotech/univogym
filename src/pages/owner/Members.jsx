...
import { getSessionCachedData, invalidateCache } from '../../utils/dataCache';
...
/**
 * AddPtPackageModal
 * Allows adding a mid-month 1-on-1 PT package to an active gym member.
 * - Starts PT on the chosen start date (defaults to today)
 * - Automatically computes PT End Date based on package duration
 * - Keeps existing Gym Floor membership active and untouched
 * - Records a financial transaction in payments (counts in today collection, revenue, transaction history)
 * - Offers instant WhatsApp Bill & Digital Receipt sharing
 */
function AddPtPackageModal({ member, gymId, onClose, onSave, trainers = [], plans = [] }) {
  const settings = getGymSettings();
  ...
  const trainerName = selectedTrainerObj ? (selectedTrainerObj.name || selectedTrainerObj.fullName) : (member.trainerName || "Assigned Coach");
      ...
      <div className="space-y-4 text-slate-800 text-xs">
        ...
        <div className="space-y-1.5">
          ...
          >
            {trainers.map((t) => (
              <option key={t.id} value={t.id}>
                {t.name || t.fullName} {t.specialization ? `(${t.specialization})` : ''} — {t.phone || ''}
              </option>
            ))}
          </select>
        </div>
...
export default function Members() {
  ...
  const [ptAddonMember, setPtAddonMember] = useState(null);
  ...
  const handlePtAddonSuccess = (memberId, updatedFields) => {
    setMembers((prev) =>
      prev.map((m) => (m.id === memberId ? { ...m, ...updatedFields } : m))
    );
  };
  ...
  return (
    <div className='space-y-6'>
                          ...
                          {isLeft ? (
                            ...
                            <>
                              <button
                                onClick={() => setPtAddonMember(m)}
                                className='inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-purple-50 hover:bg-purple-100 text-purple-700 font-bold text-xs border border-purple-200 transition shadow-sm'
                                title='Bich month me 1-on-1 PT package, validity aur bill add karein'
                              >
                                <Sparkles className='w-3.5 h-3.5 text-purple-600' />
                                <span>+ PT</span>
                              </button>
      ...
      {/* Add Mid-Month PT Package & Transaction Modal */}
      {ptAddonMember && (
        <AddPtPackageModal
