...
/**
 * Modal for WhatsApp Reminders Broadcast & Status Tracking
 */
function ReminderBlastModal({ members, onClose, onMemberReminderSent }) {
  const [filterType, setFilterType] = useState('all'); // all, partial, ending_soon, expired, overdue
  ...
  const eligibleMembers = members.filter((m) => {
    ...
    if (filterType === 'partial') return isPartial(m);
    if (filterType === 'ending_soon') return isEndingSoon(m);
    if (filterType === 'expired') return isExpired(m);
    if (filterType === 'overdue') return isOverdue(m);
    // 'all' includes anyone needing attention
  ...
  const sendReminder = (m) => {
    ...
    onMemberReminderSent(m.id);
    toast.success(`WhatsApp reminder opened for ${m.name || m.fullName}`);
  };

  const sentCount = eligibleMembers.filter((m) => !!m.lastReminderSent).length;
  const pendingCount = eligibleMembers.length - sentCount;

  return (
    <Modal
      isOpen={true}
      onClose={onClose}
      title="🔔 WhatsApp Reminders & Notice Center"
      maxWidth="max-w-2xl"
    >
      <div className="space-y-4 text-slate-800">
        {/* Top Summary Banner */}
        <div className="p-3 rounded-2xl bg-gradient-to-r from-emerald-500/10 via-teal-500/10 to-indigo-500/10 border border-emerald-200 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-emerald-600 text-white flex items-center justify-center shrink-0 shadow-sm">
              <Bell className="w-4 h-4" />
            </div>
            <div>
              <p className="font-bold text-slate-900">Track & Send WhatsApp Notices</p>
              <p className="text-[11px] text-slate-500">
                Live delivery status: kisko bhej diya (Sent) aur kisko baaki hai (Pending)
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 font-extrabold text-[11px] border border-emerald-300">
              ✓ Sent: {sentCount}
            </span>
            <span className="px-2.5 py-1 rounded-lg bg-amber-100 text-amber-900 font-extrabold text-[11px] border border-amber-300">
        ...
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-xs">
          {[
            { key: 'all', label: 'All Due & Renewals' },
            { key: 'partial', label: 'Partial Due (बाकी)' },
            { key: 'ending_soon', label: 'Ending Soon (≤3d)' },
            { key: 'expired', label: 'Expired (1-3d)' },
            { key: 'overdue', label: 'Overdue (>3d)' },
          ].map((tab) => (
            <button
              key={tab.key}
              type="button"
        ...
        {/* Member List with Live Sent / Not Sent Badges */}
        <div className="divide-y divide-slate-100 max-h-[380px] overflow-y-auto pr-1">
          {eligibleMembers.length === 0 ? (
            <div className="py-12 text-center text-slate-400 text-xs">
              🎉 Sab clear hai! Is category me koi pending reminder nahi hai.
            </div>
          ) : (
            eligibleMembers.map((m) => {
              ...
              return (
                <div key={m.id} className="py-3 flex items-center justify-between gap-3 hover:bg-slate-50/80 px-2 rounded-xl transition">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-0.5">
                        ...
                        <span>•</span>
                        {isDue ? (
                          <span className="font-bold text-amber-700">Due Balance: ₹{m.dueAmount}</span>
                        ) : (
                          <span className={daysInfo.cls.includes('red') ? 'font-bold text-red-600' : 'font-semibold text-slate-600'}>
                            {daysInfo.text}
                          </span>
                        )}
                      </div>
                  ...
                  <div className="shrink-0 flex items-center gap-2">
                    ...
                    >
                      ...
                      <span>{m.lastReminderSent ? 'Resend' : 'Send WhatsApp'}</span>
                    </button>
                  </div>
                </div>
              );
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
          <span className="text-slate-500">
            Total {eligibleMembers.length} member(s) found
          </span>
          <button
            type="button"
            onClick={onClose}
...
export default function Members() {
  ...
  const [showReminderBlast, setShowReminderBlast] = useState(false);
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        ...
        <div className='flex flex-wrap items-center gap-2'>
          <button
            onClick={() => setShowReminderBlast(true)}
            className='flex-1 sm:flex-none flex items-center justify-center gap-2 px-3.5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold transition shadow-sm shadow-amber-500/20'
            title='Open WhatsApp Notice & Reminder Center'
          >
            <Bell className='w-4 h-4' />
            <span>Reminders ({totalRemindersDue})</span>
          </button>
          <button
                    ...
                    <tr key={m.id} className='hover:bg-slate-50/70 transition items-center'>
                      ...
                      <td className='px-5 py-3.5 text-right'>
                        <div className='inline-flex items-center gap-1.5 justify-end'>
                          ...
                          {/* 6. WhatsApp Reminder Button (with Sent / Not Sent status) */}
                          {!isLeft && (
                            <button
                              onClick={() => handleSendIndividualReminder(m)}
                              className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold border transition shadow-sm ${
                                m.lastReminderSent
                              ...
                              title={
                                m.lastReminderSent
                                  ? `Reminder Sent on ${new Date(m.lastReminderSent).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} (${formatDate(m.lastReminderSent)}). Click to resend WhatsApp.`
                                  : 'Send WhatsApp Reminder / Notice'
                              }
                            >
                              <MessageSquare className={`w-3.5 h-3.5 ${m.lastReminderSent ? 'text-emerald-600' : 'text-slate-500'}`} />
                              <span>{m.lastReminderSent ? 'Sent' : 'Remind'}</span>
                            </button>
                          )}
                ...
                <div className='pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between gap-1.5'>
                  ...
                  {!isLeft && (
                    <button
                      ...
                      title={
                        m.lastReminderSent
                          ? `Reminder sent at ${new Date(m.lastReminderSent).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}. Click to resend.`
                          : 'Send WhatsApp Reminder / Notice'
                      }
                    >
                      <MessageSquare className={`w-3 h-3 ${m.lastReminderSent ? 'text-emerald-600' : 'text-slate-500'}`} />
                      <span>{m.lastReminderSent ? 'Sent' : 'Remind'}</span>
                    </button>
                  )}
      ...
      {/* WhatsApp Reminders & Notice Center Modal */}
      {showReminderBlast && (
        <ReminderBlastModal
          members={members}
          onClose={() => setShowReminderBlast(false)}
          onMemberReminderSent={handleMemberReminderSent}
        />
      )}
    </div>
  );
